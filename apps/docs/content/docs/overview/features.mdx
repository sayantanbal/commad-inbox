...
An in-app agent uses **Corsair MCP tools** to read mail, inspect schemas, and run scripts against Gmail and Calendar. Destructive **`run_script`** actions require explicit user approval in the UI.
...
- MCP adapter (`list_operations`, `get_schema`, `run_script`)
